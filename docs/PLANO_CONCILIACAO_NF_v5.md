...
> Versão 5 — atualizada em 22/04/2026
...
| `comparator.py` | Engine de comparação e categorização | 🔜 Próximo |
| `report_excel.py` | Gera .xlsx com formatação e 5 abas | ⏳ Pendente |
| `report_pdf.py` | Gera .pdf formatado para impressão | ⏳ Pendente |
...
| Aba | Conteúdo |
|-----|----------|
| **Resumo** | Totais, percentuais, dashboard visual |
| **Conciliadas** | NFs que batem perfeitamente em todos os campos |
| **Divergentes** | NFs presentes em ambos mas com algum campo diferente (mostra valor A × valor B) |
| **Só Prefeitura** | NFs que existem na prefeitura mas não estão no sistema |
| **Só Sistema** | NFs que estão no sistema mas não aparecem na prefeitura |
...
```bash
pip install streamlit pandas openpyxl xlrd reportlab
```

> `pdfplumber` não é necessário no escopo atual (sem leitura de PDF).
...
5. **Desenvolver `comparator.py`** ← estamos aqui
6. Desenvolver `report_excel.py`
7. Desenvolver `report_pdf.py`
